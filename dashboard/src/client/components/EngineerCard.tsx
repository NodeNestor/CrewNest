...
import { Play, Square, RotateCcw, Terminal, Maximize2 } from 'lucide-react';
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2.5">
          <span className={`w-2.5 h-2.5 rounded-full ${running ? 'bg-emerald-400 animate-pulse' : 'bg-gray-600'}`} />
          <div>
      ...
      <div className="mt-3 text-xs text-gray-500 space-y-1">
        <p>Image: {engineer.image}</p>
        {engineer.ssh_port && <p>SSH: localhost:{engineer.ssh_port}</p>}
        {engineer.vnc_port && <p>VNC: localhost:{engineer.vnc_port}</p>}
        {engineer.live_status?.uptime && (
          <p>Up since: {new Date(engineer.live_status.uptime).toLocaleString()}</p>
        )}
      </div>
